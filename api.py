...
async def getPartners():
    async with httpx.AsyncClient() as client:
        response = await client.get("http://localhost:8000/partners")
...
async def getPartner(partner_id):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"http://localhost:8000/partners/{partner_id}")
...
async def postPartner(new_partner):
    async with httpx.AsyncClient() as client:
        response = await client.post("http://localhost:8000/partners", json=new_partner)
...
async def updatePartner(partner_id, updated_partner):
    async with httpx.AsyncClient() as client:
        response = await client.put(f"http://localhost:8000/partners/{partner_id}", json=updated_partner)
...
async def deletePartner(partner_id):
    async with httpx.AsyncClient() as client:
        response = await client.delete(f"http://localhost:8000/partners/{partner_id}")
